...
import generate_Figure2_equivalent as base  # noqa: E402
...
def write_supporting_tables(df: pd.DataFrame, out_dir: Path, scenario_year: str) -> None:
    ...
    ratio_summary = (
        usable.groupby(["Year", "Crop"], as_index=False)
        .agg(
            n_states=("State", "nunique"),
            output_lakh_sum=("Output_lakh", "sum"),
            production_tonne_sum=("Production_tonne", "sum"),
            mean_msp_rs_per_kg=("MSP_rs_per_kg", "mean"),
            mean_state_rupee_per_kg=("rupee_per_kg", "mean"),
            mean_state_ratio_to_msp=("ratio_to_msp", "mean"),
            min_state_ratio_to_msp=("ratio_to_msp", "min"),
            max_state_ratio_to_msp=("ratio_to_msp", "max"),
        )
        .sort_values(["Year", "Crop"])
        .reset_index(drop=True)
    )
    ratio_summary["national_realized_price_rs_per_kg"] = (
        ratio_summary["output_lakh_sum"] * 100.0 / ratio_summary["production_tonne_sum"]
    )
    ratio_summary["national_ratio_to_msp"] = (
        ratio_summary["national_realized_price_rs_per_kg"] / ratio_summary["mean_msp_rs_per_kg"]
    )
    # Keep the old column names for backward compatibility, but make them reflect
    # the benchmark logic actually used in the optimization: summed value divided by
    # summed production at the crop-year level.
    ratio_summary["mean_rupee_per_kg"] = ratio_summary["national_realized_price_rs_per_kg"]
    ratio_summary["mean_ratio_to_msp"] = ratio_summary["national_ratio_to_msp"]
    ratio_summary["min_ratio_to_msp"] = ratio_summary["min_state_ratio_to_msp"]
    ratio_summary["max_ratio_to_msp"] = ratio_summary["max_state_ratio_to_msp"]
    source_breakdown = (
        df.assign(
...
def postprocess_panel_b_outputs() -> None:
    values_csv = base.OUT_DIR / "Figure2_equivalent_panel_b_values.csv"
    ...
    summary_csv = bootstrap_dir / "Figure2_equivalent_panel_b_bootstrap_summary.csv"
    iterations_csv = bootstrap_dir / "Figure2_equivalent_panel_b_bootstrap_iterations.csv"
    ...
    if summary_csv.exists():
        ...
        _build_signed_panel_b(
            summary,
            base.FIG_DIR / "Figure2_equivalent_panel_b.png",
            base.FIG_DIR / "Figure2_equivalent_panel_b.pdf",
        )
    else:
