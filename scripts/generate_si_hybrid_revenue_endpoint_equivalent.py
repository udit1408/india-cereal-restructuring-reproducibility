...
OUT_DIR = DATA_DIR / "Figure2_equivalent"

OUT_PNG = FIG_DIR / "Figure2_equivalent.png"
OUT_PDF = FIG_DIR / "Figure2_equivalent.pdf"
OUT_VALUES = OUT_DIR / "Figure2_equivalent_values.csv"
OUT_SUMMARY = OUT_DIR / "Figure2_equivalent_summary.csv"
OUT_AUDIT = OUT_DIR / "Figure2_equivalent_audit.md"
...
def summarize_display(display: pd.DataFrame, all_coverage: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["benchmark"] = summary["scenario_key"].map(
        {"MSP": "MSP benchmark", DISPLAY_SCENARIO: f"Hybrid {DISPLAY_SCENARIO} benchmark"}
    )
    return summary
...
def build_figure(summary: pd.DataFrame, crop_year_coverage: pd.DataFrame) -> None:
    ...
    for ax, strategy, panel_label in zip(
        ...
        for idx, metric in enumerate(METRIC_ORDER):
            ...
            hybrid = float(subset.loc[(metric, DISPLAY_SCENARIO), "display_pct_change"])
            ax.plot([msp, hybrid], [idx, idx], color="0.75", linewidth=1.4, zorder=1)
            ax.scatter(
                msp,
                idx,
                s=50,
    ...
    axes[1].legend(
        legend_handles,
        ["MSP benchmark", f"Hybrid {DISPLAY_SCENARIO} benchmark"],
        frameon=False,
        fontsize=8.5,
        loc="lower right",
    )
    ...
    fig.text(
        ...
        (
            ...
            f"The hybrid {DISPLAY_SCENARIO} benchmark uses matched official state-year realized prices where available "
...
def write_audit(summary: pd.DataFrame, all_values: pd.DataFrame, crop_year_coverage: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "",
        f"This SI-only figure is the Figure 2(b)-equivalent endpoint comparison built from the hybrid {DISPLAY_SCENARIO} revenue benchmark.",
    ...
    for strategy in STRATEGIES:
        ...
        lines.append("")
    lines.append("Ordering checks across hybrid year scenarios:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build the standalone Figure2_equivalent endpoint comparison using the hybrid realized-price benchmark."
    )
    parser.parse_args()
    main()
