...
LOG_FILE="$LOG_DIR/revision2_audited_batch_${RUN_STAMP}.log"
  ...
  cat > "$RUN_CONTEXT" <<EOF
{
  ...
  "mode": "$MODE",
  "runner": "revision_2/scripts/run_audited_revision2_batch.sh",
  "log_file": "logs/$(basename "$LOG_FILE")"
}
EOF
}

echo "Revision 2 audited batch runner"
echo "root: $ROOT"
...
echo
echo "Audited revision-2 batch complete."
