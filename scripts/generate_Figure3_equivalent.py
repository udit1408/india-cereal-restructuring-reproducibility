...
def build_alternative_edges(
    ...
    staple_deficits = (
        rw_edges.loc[rw_edges["diff_trade_kcal"] < 0, ["source_state", "target_state", "diff_trade_kcal"]]
        .copy()
        .assign(staple_deficit_kcal=lambda df: -df["diff_trade_kcal"])
        .groupby(["source_state", "target_state"], as_index=False)["staple_deficit_kcal"]
        .sum()
        .sort_values(["source_state", "target_state"])
        .reset_index(drop=True)
    )
    ...
    alt_edges = (
        baseline_edges.rename(columns={"quantity_kcal": "baseline_alt_trade_kcal"})
        .merge(staple_deficits, on=["source_state", "target_state"], how="outer")
        .fillna(0.0)
        .sort_values(["source_state", "target_state"])
        .reset_index(drop=True)
    )
    alt_edges["quantity_kcal"] = alt_edges["baseline_alt_trade_kcal"]
    alt_edges["is_new_link"] = (
        (alt_edges["baseline_alt_trade_kcal"] <= 0.0) & (alt_edges["staple_deficit_kcal"] > 0.0)
    )
    alt_edges["pre_scale_trade_kcal"] = alt_edges["baseline_alt_trade_kcal"] + alt_edges["staple_deficit_kcal"]

    source_summary = (
        alt_edges.groupby("source_state", as_index=False)
        .agg(
            original_alt_trade_kcal=("baseline_alt_trade_kcal", "sum"),
            staple_deficit_kcal=("staple_deficit_kcal", "sum"),
            pre_scale_alt_trade_kcal=("pre_scale_trade_kcal", "sum"),
            new_link_count=("is_new_link", "sum"),
        )
        .rename(columns={"source_state": "State"})
    )
    summary = alt_prod.merge(source_summary, on="State", how="outer").fillna(0.0)
    summary["surplus_alt_prod_kcal"] = summary["optimized_alt_prod_kcal"] - summary["original_alt_prod_kcal"]
    denom = summary["pre_scale_alt_trade_kcal"].to_numpy(dtype=float)
    numer = summary["optimized_alt_prod_kcal"].to_numpy(dtype=float)
    summary["scale_factor"] = np.minimum(
        1.0,
        np.divide(
            numer,
            denom,
    ...
    summary["optimized_alt_trade_kcal"] = summary["pre_scale_alt_trade_kcal"] * summary["scale_factor"]
    summary["capacity_limited"] = (
        (summary["pre_scale_alt_trade_kcal"] > 0.0)
        & (summary["optimized_alt_prod_kcal"] < summary["pre_scale_alt_trade_kcal"])
    )
    summary["new_link_count"] = summary["new_link_count"].round().astype(int)
        ...
        alt_edges.merge(
            summary[
                [
                    "State",
                    "scale_factor",
                    "optimized_alt_prod_kcal",
                    "pre_scale_alt_trade_kcal",
                    "optimized_alt_trade_kcal",
                    "capacity_limited",
                ]
            ],
    ...
    alt_edges["optimized_trade_kcal"] = alt_edges["pre_scale_trade_kcal"] * alt_edges["scale_factor"].fillna(0.0)
    new_links = alt_edges.loc[alt_edges["is_new_link"]].copy()
    return (
        baseline_edges,
        summary.sort_values("optimized_alt_trade_kcal", ascending=False).reset_index(drop=True),
        alt_edges,
        new_links.sort_values("optimized_trade_kcal", ascending=False).reset_index(drop=True),
    )
...
def write_audit(
    ...
    if top_new_links.empty:
        ...
        new_link_lines = [
            f"{row.source_state} -> {row.target_state}: {row.optimized_trade_kcal:.3e} kcal"
