...
STATE_PRICE_CSV = INPUT_DIR / "state_realized_price_state_year_inputs_2011_12_to_2017_18.csv"
JOIN_AUDIT_CSV = INPUT_DIR / "state_realized_price_join_audit_2011_12_to_2017_18.csv"
OUT_PNG = FIG_DIR / "si_revenue_profit_sensitivity.png"
OUT_PDF = FIG_DIR / "si_revenue_profit_sensitivity.pdf"
OUT_SCENARIOS = OUT_DIR / "si_revenue_profit_sensitivity_values.csv"
OUT_SUMMARY = OUT_DIR / "si_revenue_profit_sensitivity_summary.csv"
OUT_AUDIT = OUT_DIR / "si_revenue_profit_sensitivity_audit.md"
...
def build_figure(summary: pd.DataFrame, crop_year_coverage: pd.DataFrame) -> None:
    ...
    main_ax.set_title("a  Endpoint profit sensitivity under the realized-price benchmark", loc="left", fontweight="bold")
    ...
    main_ax.legend(
        legend_handles,
        ["MSP benchmark", "Realized-price 2017-18 benchmark"],
        frameon=False,
        fontsize=8.3,
        loc="lower right",
    )
    ...
    fig.text(
        ...
        0.012,
        (
            "Panel a shows the 2017-18 realized-price benchmark, aligned to the 2017 baseline optimization. "
...
def write_audit(values: pd.DataFrame, summary: pd.DataFrame, crop_year_coverage: pd.DataFrame) -> None:
    ...
    lines = [
        "# Revenue profit sensitivity audit",
        "",
        "This SI-only figure reruns the primary endpoint solves under a realized-price",
        ...
        "",
        "Panel a displays only the 2017-18 realized-price benchmark because this is the year",
        "most closely aligned with the 2017 baseline optimization used in the manuscript.",
        f"For 2017-18, direct realized-price coverage is {100.0 * float(hybrid_2017['direct_key_share']):.2f}% of decision keys",
        f"and {100.0 * float(hybrid_2017['direct_area_share']):.2f}% of baseline cereal area.",
        f"Across realized-price scenarios, direct realized-price coverage spans {direct_key_min:.2f}-{direct_key_max:.2f}% of decision keys",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build a supplementary realized-price/MSP-fallback profit sensitivity figure."
    )
    parser.parse_args()
    main()
