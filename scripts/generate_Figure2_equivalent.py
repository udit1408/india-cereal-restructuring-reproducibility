...
OUT_DIR = ROOT / "data" / "generated" / "Figure2_equivalent"
...
PANEL_FILES = {
    "a": FIG_DIR / "Figure2_equivalent_panel_a.png",
    "b": FIG_DIR / "Figure2_equivalent_panel_b.png",
    "c": FIG_DIR / "Figure2_equivalent_panel_c.png",
    "d": FIG_DIR / "Figure2_equivalent_panel_d.png",
}
COMPOSITE_PNG = FIG_DIR / "Figure2_equivalent.png"
COMPOSITE_PDF = FIG_DIR / "Figure2_equivalent.pdf"
SUMMARY_CSV = OUT_DIR / "Figure2_equivalent_summary.csv"
AUDIT_MD = OUT_DIR / "Figure2_equivalent_audit.md"
COVERAGE_CSV = OUT_DIR / "Figure2_equivalent_price_coverage.csv"
MANIFEST_MD = OUT_DIR / "Figure2_equivalent_manifest.md"
...
def _recompute_initial_state_revenue(
    *,
    states: list[str],
    districts_by_state: dict[str, list[str]],
    crops_by_pair: dict[tuple[str, str], list[str]],
...
def _resolve_official_value(
    *,
    ...
    direct = state_lookup.get((scenario_year, state_key, crop_key))
    if direct is not None and np.isfinite(direct) and direct > 0:
        return float(direct), "direct"

    national = national_lookup.get((scenario_year, crop_key))
    if national is None or not np.isfinite(national) or national <= 0:
        raise KeyError(
            f"Missing {label} benchmark for year={scenario_year}, state={state}, crop={crop}"
        )
    return float(national), "national_mean"


def _apply_official_benchmark_to_dict_context(
    base_context: dict[str, object],
    *,
    scenario_year: str,
    state_price_lookup: dict[tuple[str, str, str], float],
    national_price_lookup: dict[tuple[str, str], float],
    state_cost_lookup: dict[tuple[str, str, str], float],
    national_cost_lookup: dict[tuple[str, str], float],
    panel_key: str,
) -> tuple[dict[str, object], dict[str, object]]:
    context = copy.deepcopy(base_context)
    benchmark_prices: dict[tuple[str, str, str], float] = {}
    benchmark_costs: dict[tuple[str, str, str], float] = {}
    price_direct_keys = 0
    price_national_keys = 0
    cost_direct_keys = 0
    cost_national_keys = 0
    price_direct_area = 0.0
    price_national_area = 0.0
    cost_direct_area = 0.0
    cost_national_area = 0.0
    ...
    for key in context["msp_per_prod"]:
        ...
        benchmark_price, price_source = _resolve_official_value(
            scenario_year=scenario_year,
            state=state,
            crop=crop,
            state_lookup=state_price_lookup,
            national_lookup=national_price_lookup,
            label="price",
        )
        benchmark_cost, cost_source = _resolve_official_value(
            scenario_year=scenario_year,
            state=state,
            crop=crop,
            state_lookup=state_cost_lookup,
            national_lookup=national_cost_lookup,
            label="cost",
        )

        benchmark_prices[key] = benchmark_price
        benchmark_costs[key] = benchmark_cost

        if price_source == "direct":
            price_direct_keys += 1
            price_direct_area += area
        else:
            price_national_keys += 1
            price_national_area += area

        if cost_source == "direct":
            cost_direct_keys += 1
            cost_direct_area += area
        else:
            cost_national_keys += 1
            cost_national_area += area

    context["benchmark_price_per_prod"] = benchmark_prices
    context["benchmark_cost_per_prod"] = benchmark_costs
    context["msp_per_prod"] = benchmark_prices
    context["cost_per_prod"] = benchmark_costs
    context["initial_state_profit"] = _recompute_initial_state_profit(
        ...
        price_map=benchmark_prices,
        cost_per_prod=benchmark_costs,
    )
    context["initial_state_msp"] = _recompute_initial_state_revenue(
        states=context["states"],
        districts_by_state=context["districts_by_state"],
        crops_by_pair=context["crops_by_pair"],
        current_cereal_area=context["current_cereal_area"],
    ...
    total_keys = price_direct_keys + price_national_keys
    total_area = price_direct_area + price_national_area
    coverage = {
        "panel": panel_key,
        "season": str(context["season"]),
        "scenario_year": scenario_year,
        "price_direct_keys": price_direct_keys,
        "price_national_keys": price_national_keys,
        "cost_direct_keys": cost_direct_keys,
        "cost_national_keys": cost_national_keys,
        "direct_keys": price_direct_keys,
        "national_mean_keys": price_national_keys,
        "fallback_keys": 0,
        "direct_key_share": price_direct_keys / total_keys if total_keys else np.nan,
        "national_mean_key_share": price_national_keys / total_keys if total_keys else np.nan,
        "fallback_key_share": 0.0,
        "direct_area_share": price_direct_area / total_area if total_area else np.nan,
        "national_mean_area_share": price_national_area / total_area if total_area else np.nan,
        "fallback_area_share": 0.0,
        "price_direct_key_share": price_direct_keys / total_keys if total_keys else np.nan,
        "price_national_key_share": price_national_keys / total_keys if total_keys else np.nan,
        "cost_direct_key_share": cost_direct_keys / total_keys if total_keys else np.nan,
        "cost_national_key_share": cost_national_keys / total_keys if total_keys else np.nan,
        "price_direct_area_share": price_direct_area / total_area if total_area else np.nan,
        "price_national_area_share": price_national_area / total_area if total_area else np.nan,
        "cost_direct_area_share": cost_direct_area / total_area if total_area else np.nan,
        "cost_national_area_share": cost_national_area / total_area if total_area else np.nan,
    }
    return context, coverage


def _apply_official_benchmark_to_season_context(
...
def _build_panel_a(
    *,
    ...
    for season, notebook_name in SEASON_NOTEBOOKS.items():
        ...
        context, coverage = _apply_official_benchmark_to_dict_context(
            base_context,
            scenario_year=scenario_year,
    ...
    kharif.to_csv(OUT_DIR / "Figure2_equivalent_panel_a_kharif_by_alpha.csv", index=False)
    rabi.to_csv(OUT_DIR / "Figure2_equivalent_panel_a_rabi_by_alpha.csv", index=False)
    combined.to_csv(OUT_DIR / "Figure2_equivalent_panel_a_combined_by_alpha.csv", index=False)
    ...
    _plot_combined(
        ...
        stem="Figure2_equivalent_panel_a",
    )
    _plot_combined(
        combined,
        FIG_DIR,
        ...
        stem="Figure2_equivalent_panel_a_pct_2017_baseline",
    )
...
def _build_panel_b_whisker_figure(summary: pd.DataFrame, out_png: Path, out_pdf: Path) -> None:
    ...
    if "display_metric" in summary.columns:
        base = (
            summary.drop_duplicates(subset=["metric"])
...
def _write_panel_b_bootstrap_audit(
    *,
    ...
    lines = [
        ...
        "",
        f"This bootstrap adds whiskers to the primary Figure 2(b) rebuild built with the official {scenario_year} realized-price and prod_cost_c2rev benchmark.",
        "The revenue and cost benchmark are held fixed across iterations. Uncertainty is propagated by resampling",
        ...
        "the official price-and-cost benchmark, rather than a second ad hoc uncertainty model for benchmark inputs.",
        "For plotting, the empirical bootstrap spread is centered on the deterministic bar value so the whiskers",
        "show uncertainty around the reported estimate while preserving the bootstrap interval width.",
        "",
    ...
    (PANEL_B_BOOTSTRAP_DIR / "Figure2_equivalent_panel_b_bootstrap_audit.md").write_text(
        "\n".join(lines).rstrip() + "\n",
        encoding="utf-8",
    )
...
def _build_panel_b_with_bootstrap(
    *,
    ...
    reproduction.to_csv(
        PANEL_B_BOOTSTRAP_DIR / "Figure2_equivalent_panel_b_deterministic_reproduction_check.csv",
        index=False,
    )
    iterations.to_csv(
        PANEL_B_BOOTSTRAP_DIR / "Figure2_equivalent_panel_b_bootstrap_iterations.csv",
        index=False,
    )
    summary.to_csv(
        PANEL_B_BOOTSTRAP_DIR / "Figure2_equivalent_panel_b_bootstrap_summary.csv",
        index=False,
    )

    _build_panel_b_whisker_figure(
        summary,
        FIG_DIR / "Figure2_equivalent_panel_b.png",
        FIG_DIR / "Figure2_equivalent_panel_b.pdf",
    )
...
def _build_panel_b(
    *,
    ...
    table.to_csv(OUT_DIR / "Figure2_equivalent_panel_b_values.csv", index=False)
    write_latex_table(table, OUT_DIR / "Figure2_equivalent_panel_b_values.tex")
...
def _prepare_panel_b_display_table(table: pd.DataFrame) -> pd.DataFrame:
    out = table.copy()
    out["display_metric"] = (
        out["display_metric"].astype(str)
        if "display_metric" in out.columns
        else out["metric"].astype(str)
    )
    mask = (out["metric"] == "Net return") & out["original_total"].lt(0)
    if not mask.any():
        return out

    baseline_deficit = (-out.loc[mask, "original_total"]).clip(lower=0.0)
    optimized_deficit = (-out.loc[mask, "optimized_total"]).clip(lower=0.0)
    valid = baseline_deficit.gt(0)

    pct_reduction = pd.Series(0.0, index=baseline_deficit.index, dtype=float)
    pct_reduction.loc[valid] = (
        100.0 * (baseline_deficit.loc[valid] - optimized_deficit.loc[valid]) / baseline_deficit.loc[valid]
    )

    out.loc[mask, "display_metric"] = "Net-return deficit"
    out.loc[mask, "pct_reduction"] = pct_reduction.to_numpy()
    out.loc[mask, "display_pct_change"] = -pct_reduction.to_numpy()
    return out
...
def _build_panel_c(
    *,
    ...
    for season, config in CULTURAL_NOTEBOOKS.items():
        ...
        context, coverage = _apply_official_benchmark_to_season_context(
            base_context,
            scenario_year=scenario_year,
            state_price_lookup=state_price_lookup,
            national_price_lookup=national_price_lookup,
    ...
    kharif.to_csv(OUT_DIR / "Figure2_equivalent_panel_c_kharif.csv", index=False)
    rabi.to_csv(OUT_DIR / "Figure2_equivalent_panel_c_rabi.csv", index=False)
    combined.to_csv(OUT_DIR / "Figure2_equivalent_panel_c_combined.csv", index=False)

    original_fig_dir = figure2c_mod.FIG_DIR
    try:
        figure2c_mod.FIG_DIR = FIG_DIR
        figure2c_mod.plot_nominal_panel(combined, output_stem="Figure2_equivalent_panel_c")
    finally:
...
def _build_panel_d(
    *,
    ...
    combined_areas.to_csv(OUT_DIR / "Figure2_equivalent_panel_d_optimized_areas.csv", index=False)
    combined_state_audit.to_csv(OUT_DIR / "Figure2_equivalent_panel_d_state_constraint_audit.csv", index=False)
    combined_district_audit.to_csv(OUT_DIR / "Figure2_equivalent_panel_d_district_area_audit.csv", index=False)
    crop_summary.to_csv(OUT_DIR / "Figure2_equivalent_panel_d_crop_summary.csv", index=False)
    transition_long.to_csv(OUT_DIR / "Figure2_equivalent_panel_d_transition_long.csv", index=False)
    transition_matrix.to_csv(OUT_DIR / "Figure2_equivalent_panel_d_transition_matrix.csv")
    ...
    try:
        ...
        figure2d_mod.plot_alluvial(
            ...
            crop_summary,
            title=None,
            output_stem="Figure2_equivalent_panel_d",
        )
    finally:
...
def _write_audit(
    *,
    ...
    lines = [
        "# Figure2_equivalent audit",
        "",
        f"This standalone figure reruns all Figure 2 panels on the primary no-hard-cap branch using the official {scenario_year} realized-price benchmark and the official prod_cost_c2rev cost benchmark.",
        "Matched state-crop combinations use direct official state-year values where available; all remaining state-crop combinations use the corresponding national crop-year benchmark from the same source family.",
        "",
        "Benchmark coverage by panel and season:",
    ]
    for row in coverage.sort_values(["panel", "season"]).itertuples(index=False):
        lines.append(
            f"- panel {row.panel} | {row.season}: direct price coverage = {100.0 * float(row.price_direct_key_share):.2f}% of decision keys ({100.0 * float(row.price_direct_area_share):.2f}% of baseline cereal area); direct cost coverage = {100.0 * float(row.cost_direct_key_share):.2f}% of decision keys ({100.0 * float(row.cost_direct_area_share):.2f}% of baseline cereal area)"
        )
    ...
    for scenario in ("Water based", "Nitrogen based"):
        ...
        net_return_change = float(subset.loc[subset["metric"] == "Net return", "pct_reduction"].iloc[0])
        calorie_change = float(subset.loc[subset["metric"] == "Calorie", "pct_reduction"].iloc[0])
        lines.append(
            f"- {scenario}: water {water_change:.3f}%, nitrogen surplus {nitrogen_change:.3f}%, net return {net_return_change:.3f}%, calorie {calorie_change:.3f}%"
        )
...
def _write_manifest() -> None:
    lines = [
        ...
        "",
        "Panel outputs:",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_a.png'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_a.pdf'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_a_pct_2017_baseline.png'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_a_pct_2017_baseline.pdf'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_b.png'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_b.pdf'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_c.png'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_c.pdf'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_d.png'}",
        f"- {FIG_DIR / 'Figure2_equivalent_panel_d.pdf'}",
        "",
        "Tabular outputs:",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_a_kharif_by_alpha.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_a_rabi_by_alpha.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_a_combined_by_alpha.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_b_values.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_b_values.tex'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_c_kharif.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_c_rabi.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_c_combined.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_d_optimized_areas.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_d_state_constraint_audit.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_d_district_area_audit.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_d_crop_summary.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_d_transition_long.csv'}",
        f"- {OUT_DIR / 'Figure2_equivalent_panel_d_transition_matrix.csv'}",
        f"- {SUMMARY_CSV}",
        f"- {COVERAGE_CSV}",
    ]
    if PANEL_B_BOOTSTRAP_DIR.exists():
        lines.extend(
            [
                f"- {PANEL_B_BOOTSTRAP_DIR / 'Figure2_equivalent_panel_b_deterministic_reproduction_check.csv'}",
                f"- {PANEL_B_BOOTSTRAP_DIR / 'Figure2_equivalent_panel_b_bootstrap_iterations.csv'}",
                f"- {PANEL_B_BOOTSTRAP_DIR / 'Figure2_equivalent_panel_b_bootstrap_summary.csv'}",
            ]
        )
    lines.extend(
        [
    ...
    if PANEL_B_BOOTSTRAP_DIR.exists():
        lines.append(f"- {PANEL_B_BOOTSTRAP_DIR / 'Figure2_equivalent_panel_b_bootstrap_audit.md'}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate the primary Figure 2 rebuild under the official realized-price and prod_cost_c2rev benchmark."
    )
    parser.add_argument(
        "--scenario-year",
        default=DEFAULT_SCENARIO_YEAR,
        help="Official benchmark year to apply for realized prices and prod_cost_c2rev (default: 2017-18).",
    )
    parser.add_argument(
