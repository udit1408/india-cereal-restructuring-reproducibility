...
def summarize(values: pd.DataFrame, coverage: pd.DataFrame) -> pd.DataFrame:
    profit_rows = values[values["metric"] == "Net return"].copy()
    coverage_summary = (
...
def build_figure(summary: pd.DataFrame, crop_year_coverage: pd.DataFrame) -> None:
    ...
    main_ax.set_title("a  Endpoint net-return sensitivity under the hybrid revenue benchmark", loc="left", fontweight="bold")
    main_ax.set_xlabel("Net-return change relative to scenario-specific baseline (%)")
...
def write_audit(values: pd.DataFrame, summary: pd.DataFrame, crop_year_coverage: pd.DataFrame) -> None:
    ...
    lines = [
        "# Hybrid revenue net-return sensitivity audit",
        "",
        ...
        "",
        "Net-return outcomes by scenario and strategy:",
    ]
    for row in summary.itertuples(index=False):
        if row.scenario_key == "MSP":
            lines.append(
                f"- {row.scenario_label} | {row.strategy}: net-return change {row.display_pct_change:.3f}% ; "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build a supplementary hybrid realized-price/MSP-fallback net-return sensitivity figure."
    )
    parser.parse_args()
    main()
