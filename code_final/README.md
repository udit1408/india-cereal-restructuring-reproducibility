...
The final workflow runs only the audited code path listed in `FINAL_CODE_MANIFEST.md`. It rebuilds the manuscript-facing figure and source-data assets under the primary 2017--18 realized-price revenue benchmark:
...
- exact SHA-256 synchronization of manuscript figure targets;
...
`METHOD_NOTATION_MAP.md` maps the manuscript notation to the final code variables and functions. The most important alignment points are:
...
- the manuscript coefficient `\pi_{j,c}` corresponds to the code expression `msp_per_prod - cost_per_prod`, with unit conversion handled by the code's `0.01` factor because the code uses kg ha-1 yields and Rs quintal-1 prices.
